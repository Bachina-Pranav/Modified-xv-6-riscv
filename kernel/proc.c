...
void mlfq(struct cpu *c)
{
  ...
  for (int qpos = 0; qpos < 5; qpos++)
  {
    while (queueInfo.size[qpos] > 0)
    {
      ...
      bestproc->no_of_times_scheduled++;
...
void procdump(void)
{
...
#if defined(FCFS) || defined(ROUNDROBIN) || defined(LBS)
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
    printf("%d\t%d\t\t%s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n", p->pid, p->queue_pos, state, p->run_time, ticks - p->entered_queue, p->no_of_times_scheduled, p->queue_time[0], p->queue_time[1], p->queue_time[2], p->queue_time[3], p->queue_time[4]);
    printf("\n");
  }
#endif
